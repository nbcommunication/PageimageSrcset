	...
	public function getInputfields() {
		...
		if($module->defaultSets) {
			...
			foreach($module->getSets() as $rule => $dimensions) {
				$attr[] = "image.{$dimensions[0]}x{$dimensions[1]}.jpg $rule";
			}
			...
			$preview .= "/* " . $this->_("Please configure the default set rules") . "\n\n" .
				...
				$this->_("Example") . ":\n" .
				implode("\n", ["320", "480", "640x480 768w", "1240", "2048 2x"]) . "\n" .
				"*/";
		}
		...
		$fieldset->label = $this->_("Mobile/Tablet Portrait Images");
		...
		$fieldset->add([
			"type" => "integer",
			"name" => "portraitWidth",
			"label" => $this->_("Maximum Width"),
			"description" => $this->_("The maximum width for generating portrait images."),
			"notes" => $this->_("Any set rules up to and including this width will have portrait variations generated."),
			"columnWidth" => 50,
			"min" => 320,
			"max" => 1680,
			"size" => 0,
